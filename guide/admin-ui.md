# 管理员后台

截图版本：`0.3.8`。截图仅用于界面参考，实际功能和布局以当前版本为准。

## 登录

### 登录

![后台登录截图](/screenshots/admin/login.png)

## 工作台

### 工作台

![工作台截图](/screenshots/admin/dashboard.png)

## 用户

### 用户管理

![用户管理截图](/screenshots/admin/users.png)

### 播放统计

![播放统计截图](/screenshots/admin/playback.png)

## 媒体

### 求片管理

![求片管理截图](/screenshots/admin/media-requests.png)

## 运营

### 卡密管理

![卡密管理截图](/screenshots/admin/cards.png)

### 积分中心

积分中心用于查看积分概览、积分流水和用户排行。

### 用户组

用户组支持 `手动用户组` 和 `智能用户组`。手动用户组由管理员维护成员；智能用户组按归类规则实时计算成员。

`有效期剩余天数` 只匹配当前有效的普通会员；可选择“小于”“等于”或“大于”指定天数，并在同一 `AND` 条件组组合为区间。用户管理可通过既有的“用户组”筛选复用这些规则，无需额外的剩余天数筛选项。

编辑用户组时可配置组权益：

- `求片次数`：留空时继承求片基础配置；填写后必须高于基础次数
- `签到最小积分` 与 `签到最大积分`：必须同时填写；留空时继承基础配置，填写的范围不能低于基础范围

一个用户命中多个用户组时，求片次数取所有命中组中的最高值；签到奖励只采用一个范围，优先选择最大奖励较高的范围，若最大奖励相同则选择最小奖励较高的范围。

### 线路管理

![线路管理截图](/screenshots/admin/server-lines.png)

### 工单管理



### 公告管理

![公告管理截图](/screenshots/admin/announcements.png)

## 运维

### 配置中心

配置中心统一管理基础、主题、通知、工单、求片、Emby 和 Telegram 配置。

### 基础

![基础截图](/screenshots/admin/system.png)

### 主题

主题配置用于自定义用户端主题色、背景和卡片透明度。

### 通知

![通知截图](/screenshots/admin/notifications.png)

### 工单

工单配置用于维护工单类型、启用状态、排序和 Telegram 接收管理员。

### 求片

![求片截图](/screenshots/admin/media-request-settings.png)

### Emby

![Emby 截图](/screenshots/admin/emby.png)

### Telegram

![Telegram 截图](/screenshots/admin/telegram.png)

### 异步任务

![异步任务截图](/screenshots/admin/tasks.png)

### 运行日志

![运行日志截图](/screenshots/admin/runtime-logs.png)

### 管理员

![管理员截图](/screenshots/admin/admin-users.png)

## 个人设置

个人设置位于后台底部账号菜单中，不属于配置中心。

![个人设置截图](/screenshots/admin/profile.png)
